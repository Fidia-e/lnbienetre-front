/* eslint-disable indent */
/* eslint-disable quotes */
const ClientsReviews = [
  {
    id: 1,
    name: 'Céline Croso',
    alt: 'Avatar de Céline',
    review:
      'Bonjour Hélène, merci infiniment pour ces photos qui motivent encore plus! \nA mon tour de vous remercier du plus profond du coeur. ...',
    more: "Bonjour Hélène, merci infiniment pour ces photos qui motivent encore plus! \nA mon tour de vous remercier du plus profond du coeur. \nGrâce à vous à votre écoute à votre bienveillance j'en suis là aujourd'hui. Je me croyais perdue, et votre soutien m'est d'une précieuse aide votre motivation à mon égard fait que j'avance à petit pas mais toujours motivée. \n15 jours avant notre rencontre j'étais à 82kg. \nLe jour où j'ai débuté avec vous j'étais à 75kg.\nJe finis ma première cure à 70.4kg! \nMerci 🙏. \nMerci Adelyne sans elle je ne serais pas où j'en suis aujourd'hui merci du fonc du coeur !",
  },
  {
    id: 2,
    name: 'Emeline Leca',
    alt: "Avatar d'Emeline",
    review:
      "J'ai terminé une cure de 5 séances de madero et je suis ravie des résultats sur ma silhouette ! De plus Hélène est une praticienne très douce, ...",
    more: "J'ai terminé une cure de 5 séances de madero et je suis ravie des résultats sur ma silhouette ! De plus Hélène est une praticienne très douce, à l'écoute et agréable. Ça a été un plaisir, je recommande à 100% ☺️",
  },
  {
    id: 3,
    name: 'Naouelle Flih',
    alt: 'Avatar de Naouelle',
    review:
      "Je suis actuellement en séances de drainage lymphatique avec Hélène. \nLes séances me font un bien fou 😍😍. C'est un pure moment, ...",
    more: "\nJe suis actuellement en séances de drainage lymphatique avec Hélène. \nLes séances me font un bien fou 😍😍. \nC'est un pure moment, ou je me sens complètement déconnectée. \nHélène est une professionnelle, très douce et à l'écoute. \nC'est une magicienne. \nJe recommande vivement et je reprendrai une cure c'est certain.",
  },
  {
    id: 4,
    name: 'Titel Moussa',
    alt: 'Avatar de Titel',
    review:
      "Merci à Hélène pour son accueil, son professionnalisme et sa douceur ! Massage tonique de la maderotherapie qu'elle maîtrise à la perfection, les résultats ...",
    more: "Merci à Hélène pour son accueil, son professionnalisme et sa douceur ! Massage tonique de la maderotherapie qu'elle maîtrise à la perfection, les résultats sont visibles après une cure de 5 séances et +. Ciao la cellulite et la rétention d'eau. Je recommande à 200% Et je reviendrais",
  },
  {
    id: 5,
    name: 'Johana Mollet',
    alt: 'Avatar de Johana',
    review:
      "Hélène est très professionnelle, elle prend le temps de s'occuper de vous et de répondre à vos objectifs. J'ai été bluffé par le résultat suite à ma première ...",
    more: "Hélène est très professionnelle, elle prend le temps de s'occuper de vous et de répondre à vos objectifs. J'ai été bluffé par le résultat suite à ma première séace (combo madérothérapie et drainge lymphatique); ca a été instantané. Au delà de l'esthétique (netteté de la peau) je suis beaucoup moins ballonnée. \nDe plus, c'est un moment hyper agréable et relaxant, on se recentre sur soi. \nIl y a un réel rapport qualité/prix. Je vous recommande Hélène les yeux fermés.",
  },
  {
    id: 6,
    name: 'Roxanne',
    alt: 'Avatar de Roxanne',
    review:
      "Le soin Ice Mdéro ! 😱 c'est une “tuerie“. Soin minceur avec un résultat dès la 1er séance ! Je suis bluffée 😍 \nC'est un soin très agréable ...",
    more: "Le soin Ice Mdéro ! 😱 c'est une “tuerie“. Soin minceur avec un résultat dès la 1er séance ! Je suis bluffée 😍 \n \nC'est un soin très agréable et qui détend. Pour ce qui aurait peur, ça ne fait pas mal et ce n'est pas gelé (comme on pourrait penser). C'est une légère sensation de fraîcheur qui est très plaisant.",
  },
  {
    id: 7,
    name: 'Yousra',
    alt: 'Avatar de Yousra',
    review:
      "Hélène a des doigts de fée! J'ai fait une cure de drainage lymphatique avec Hélène suite à une liposuccion et je peux vous dire que je regrette de ne pas ...",
    more: "Hélène a des doigts de fée! J'ai fait une cure de drainage lymphatique avec Hélène suite à une liposuccion et je peux vous dire que je regrette de ne pas être passé entre ses mains avant tellement le résultat et le bien être était immédiat des la première séance. \nHélène est une vraie professionnele et passionné par ce qu'elle fait, j'ai d'ailleurs enchaîné sur une cure de madero. \nDe plus (et pas des moindres), c'est une personne très sympatique et agréable. \nJe la recommande les yeux fermés pourtant je suis une personne plutôt exigeante. Mille merci Hélène pour le bien être que vous m'apportez chaque semaine!",
  },
];

export default ClientsReviews;
